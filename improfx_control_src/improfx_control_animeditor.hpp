...
public:
	bool PlayerRunSample(AnimCoordSample& CoordParam) {
	...
	void DrawEditorWindow(
		...
		for (size_t i = 0; i < sample.size() - 1; ++i) {
			...
			for (size_t j = 0; j < 3; ++j) {
				if (RenderCubicBezierCurve(
					...
					PlayerSample.AnimSamplePoints[j] = MedianY - SamplePointValue[j];
		...
		ImGui::TextColored(AsixColors[0], "Xaxis: %.2f", PlayerSample.AnimSamplePoints[0]);
		ImGui::TextColored(AsixColors[1], "Yaxis: %.2f", PlayerSample.AnimSamplePoints[1]);
		ImGui::TextColored(AsixColors[2], "Zaxis: %.2f", PlayerSample.AnimSamplePoints[2]);
